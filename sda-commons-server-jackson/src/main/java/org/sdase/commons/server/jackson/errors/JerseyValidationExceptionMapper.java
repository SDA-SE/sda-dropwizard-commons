 ...
 * For each {@link javax.validation.ConstraintViolation}, one InvalidParam entry is generated. As error code, the validation
 * name is used as Error Code
 */
@Provider
public class JerseyValidationExceptionMapper implements ExceptionMapper<JerseyViolationException> {
   ...
   public Response toResponse(JerseyViolationException e) {
      ...
      e.getConstraintViolations().forEach(cv ->  {
         ...
         String annotation = cv.getConstraintDescriptor().getAnnotation().annotationType().toString();

         ApiInvalidParam invalidParameter = new ApiInvalidParam(
               propertyPath,
               cv.getMessage(),
               CaseFormat.UPPER_CAMEL.to(
                     CaseFormat.UPPER_UNDERSCORE, annotation.substring(annotation.lastIndexOf('.')+1)
               )
         );
      ...
      while (iterator.hasNext()) {
         ...
         if (ElementKind.METHOD == node.getKind()) {
            ...
            if (lastClazz == null && method != null && parameter != null) { // NOSONAR
               ...
               Optional<? extends Class<?>> first = Stream.of(lastClazz.getDeclaredFields()).filter(f -> f.getName().equalsIgnoreCase(fieldname)).map(Field::getType).findFirst();
   ...
   private Optional<String> getPropertyName(Path.PropertyNode propertyNode, Class<?> parameterBeanClass) {
