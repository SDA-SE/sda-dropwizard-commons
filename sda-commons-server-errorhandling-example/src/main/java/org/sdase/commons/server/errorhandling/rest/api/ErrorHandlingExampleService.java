...
public interface ErrorHandlingExampleService {
   ...
   /**
    * Example for throwing a subclass of {@link javax.ws.rs.WebApplicationException}
    * These comprise exceptions for nearly all used http error status codes
   ...
   /**
    * Example for creating a response object that contains an {@link org.sdase.commons.shared.api.error.ApiError}
    * as entity so that the rest guide compliant error structure is returned
    ...
    * exceptions with http status codes that are not covered by {@link javax.ws.rs.WebApplicationException}s
    ...
    * @return ok, if valid. {@link javax.validation.ValidationException} if the input param is empty or not uppercase
    */
   @POST
   @Path("validation")
   @Consumes(MediaType.APPLICATION_JSON)
