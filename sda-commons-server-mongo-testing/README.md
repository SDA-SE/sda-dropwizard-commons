...
```
testCompile 'org.sdase.commons:sda-commons-server-mongo-testing'
```
...
If `EMBEDDED_MONGO_DOWNLOAD_PATH` is set to `http://example.com/download/`, the rule/extension for example
...
To test specific scenarios, e.g. a real database set up like in production, the rule/extension can be
...
If such an environment variable is set, the `MongoDbRule`/ `MongoDbClassExtension` will not start a local MongoDB but
