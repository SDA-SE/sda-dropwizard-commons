...
class S3ClassExtensionTest {
  ...
  @BeforeEach
  void setUp() {
    s3Client = S3_EXTENSION.newClient();
  ...
  void shouldResetAll() {
    ...
    var newBucketExistsResponse =
        s3Client.headObject(HeadObjectRequest.builder().bucket("new").key("data").build());
    assertThat(newBucketExistsResponse.sdkHttpResponse().isSuccessful()).isFalse();
