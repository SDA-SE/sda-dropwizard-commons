...
public class MyServiceWithMetrics {
   ...
   // Create a Histogram to track execution times. Note that histogram metric names should use the unit of the
   // recorded value as suffix. Usually durations are tracked in seconds. Fractional seconds can be tracked as
   // double value.
   ...
   private Counter someOperationSuccessCounter = Counter
         .build("some_operation_success_counter", "Counts successes occurred when some operation is invoked.")
         .register();

   // Create a Counter to count events.
   private Counter someOperationErrorCounter = Counter
         .build("some_operation_error_counter", "Counts errors occurred when some operation is invoked.")
         .register();

   // Create a Gauge to track the most recent value of a metric. The name of a gauge metric should use the unit of the
   // recorded value as suffix.
   private Gauge someOperationCurrentValueGauge = Gauge
         .build("some_operation_temperature_celsius", "Tracks the temperature recorded within the operation.")
         .register();
