...
public class KafkaExampleProcuderIT {
   ...
   private static final DropwizardAppRule<KafkaExampleConfiguration> DROPWIZARD_APP_RULE = new DropwizardAppRule<>(
         KafkaExampleProducerApplication.class, ResourceHelpers.resourceFilePath("test-config-producer.yml"));
   ...
   public void testUseProducerWithConfiguration() {
      ...
      await()
            .atMost(10, TimeUnit.SECONDS)
            .until(() -> !KAFKA.getKafkaTestUtils().consumeAllRecordsFromTopic("exampleTopicConfiguration").isEmpty());

      ConsumerRecord<byte[], byte[]> record = KAFKA
            .getKafkaTestUtils()
