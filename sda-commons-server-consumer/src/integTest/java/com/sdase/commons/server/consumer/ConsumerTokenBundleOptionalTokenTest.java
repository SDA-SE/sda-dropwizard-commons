...
import com.sdase.commons.server.consumer.test.ConsumerTokenOptionalTestApp;
...
public class ConsumerTokenBundleOptionalTokenTest {
   ...
   private static DropwizardAppRule<ConsumerTokenTestConfig> DW = new DropwizardAppRule<>(
         ConsumerTokenTestApp.class, ResourceHelpers.resourceFilePath("test-config.yaml"));

   private static DropwizardAppRule<ConsumerTokenTestConfig> DW_OPTIONAL = new DropwizardAppRule<>(
         ConsumerTokenOptionalTestApp.class, ResourceHelpers.resourceFilePath("test-config.yaml"));

   @ClassRule
   public static RuleChain CHAIN = RuleChain.outerRule(new EnvironmentRule().setEnv("CONSUMER_TOKEN_OPTIONAL", "true"))
         .around(DW).around(DW_OPTIONAL);
   ...
   public void shouldNotRejectOptionsRequestFixedConfig() {
      Response response = DW_OPTIONAL.client().target("http://localhost:" + DW_OPTIONAL.getLocalPort()).request().options();
      assertThat(response.getStatus()).isEqualTo(200);
   }
}
