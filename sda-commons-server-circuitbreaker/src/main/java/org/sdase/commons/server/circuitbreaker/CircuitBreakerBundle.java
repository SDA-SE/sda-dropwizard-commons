...
/**
 * Bundle that provides access to an implementation of the circuit breaker
 * pattern to handle failures of downstream services. The bundle uses
 * resilience4j and exposes instances of {@code CircuitBreaker}.
 *
 * <p>
 * The bundle allows to create circuit breakers, wrap classes in circuit
 * breakers and registers prometheus metrics.
 */
public class CircuitBreakerBundle<T extends Configuration> implements ConfiguredBundle<T> {
   ...
   private CircuitBreakerBundle(CircuitBreakerConfigurationProvider<T> configurationProvider,
         Class<? extends Throwable>[] recordedErrorClasses, Class<? extends Throwable>[] ignoredErrorClasses) {
   ...
   public void run(T configuration, Environment environment) {
      ...
      CircuitBreakerMetricsCollector circuitBreakerMetricsCollector = ofCircuitBreakerRegistry(registry);
      collectorRegistry.register(circuitBreakerMetricsCollector);

      environment.lifecycle().manage(onShutdown(() -> collectorRegistry.unregister(circuitBreakerMetricsCollector)));
   }
    ...
    * 
    * @param name
    *           The name of the circuit breaker, used in the metrics. Multiple
    *           calls with the same name return the same circuit breaker
   ...
   public interface ConfigurationBuilder {
      ...
      /**
       * Provide a custom default configuration for circuit breakers created
       * using the bundle.
       * 
       * @param config
       ...
       */
      <T extends Configuration> ExceptionBuilder<T> withCustomConfig(CircuitBreakerConfiguration config);

      /**
       * Use a provider for a custom default configuration for circuit breakers
       * created using the bundle.
       * 
       * @param provider
       *           Provider to extract the circuit breaker configuration from
       *           the Dropwizard configuration.
       * @param <T>
       *           Type of the Dropwizard configuration.
       * @return the same builder instance
       */
      <T extends Configuration> ExceptionBuilder<T> withConfigProvider(CircuitBreakerConfigurationProvider<T> provider);

      /**
       * Use the default configuration for circuit breakers created using the
       * bundle.
       * 
       * @return the same builder instance
       */
   ...
   public interface ExceptionBuilder<T extends Configuration> extends FinalBuilder<T> {
      ...
      /**
       * Configures a list of error classes that are recorded as a failure and
       * thus increase the failure rate. Any exception matching or inheriting
       * from one of the list should count as a failure, unless ignored
       * 
       * @param errorClasses
      ...
      /**
       * Configures a list of error classes that are ignored as a failure and
       * thus do not increase the failure rate. Any exception matching or
       * inheriting from one of the list will not count as a failure, even if
       * marked via record.
       *
       * @param errorClasses
   ...
   public static class Builder<C extends Configuration>
      ...
      @Override
      public <T extends Configuration> ExceptionBuilder<T> withCustomConfig(CircuitBreakerConfiguration config) {
      ...
      public CircuitBreakerBundle<C> build() {
         return new CircuitBreakerBundle<>(configurationProvider, recordedErrorClasses, ignoredErrorClasses);
      }
   }
}
