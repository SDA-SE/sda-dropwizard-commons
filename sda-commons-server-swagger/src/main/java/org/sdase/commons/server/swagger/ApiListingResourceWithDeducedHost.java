...
public class ApiListingResourceWithDeducedHost extends ApiListingResource {
   ...
   /**
    * Creates a new resource with an instance id that is used as a key for the
    * registration of the swagger internal caches.
    *
    * @param instanceId
    *           the instanceId to be used for the cache keys.
    */
   ...
   protected Swagger process(Application app, ServletContext servletContext, ServletConfig sc, HttpHeaders headers,
         UriInfo uriInfo) {
      Swagger swagger = super.process(app, servletContext, new DelegatingServletConfig(sc, instanceId), headers,
            uriInfo);
   ...
   /**
    * A customized {@link ServletConfig} that proxies the request to an
    * originally provided instance. However, for some init params, it replaces
    * the value with a custom one.
    */
   private static class DelegatingServletConfig implements ServletConfig {
       ...
       *
       * @param servletConfig
       *           the original config that should be used
       * @param id
       *           the id that should be used to create the config, context, and
       *           scanner id.
       */
