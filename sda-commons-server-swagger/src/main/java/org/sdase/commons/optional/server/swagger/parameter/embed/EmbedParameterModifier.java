...
/**
 * Adds the embeddable resources as query parameter such that they can be
 * selected in swagger.
 */
@SwaggerDefinition
public class EmbedParameterModifier implements ReaderListener {
   ...
   public void afterScan(Reader reader, Swagger swagger) {
      ...
      if (definitions != null && swagger.getPaths() != null) {
         swagger.getPaths().forEach((key, path) -> path.getOperations().forEach(o -> updateOperations(o, definitions)));
      }
   }
   ...
   private QueryParameter getEmbedQueryParameter(ObjectProperty embeddedObjectProperty) {
      if (embeddedObjectProperty.getProperties() != null) {
         List<String> embeddableObjects = embeddedObjectProperty
               .getProperties()
               .keySet()
               .stream()
               .sorted()
         ...
         return new QueryParameter()
               ...
               .name("embed")
               .description("Select linked resources that should be resolved and embedded into the response")
               .collectionFormat("multi")
