...
import org.sdase.commons.server.dropwizard.bundles.test.HealthcheckApplication;
import org.sdase.commons.server.dropwizard.healthcheck.helper.ExternalServiceHealthCheck;
...
class HealthCheckIT {
  ...
  static final DropwizardAppExtension<Configuration> EXT =
      new DropwizardAppExtension<>(HealthcheckApplication.class, null, randomPorts());
  ...
  void testAllHealthChecks() {
    try (Response response = healthCheckGet("/healthcheck")) {
      ...
      assertThat(metrics)
          .contains(HealthcheckApplication.DUMMY_EXTERNAL)
          .contains(HealthcheckApplication.DUMMY_INTERNAL);
    }
  }
  ...
  void testHealthChecksInternal() {
    try (Response response = healthCheckGet("/healthcheck/internal")) {
      ...
      assertThat(metrics)
          .doesNotContain(HealthcheckApplication.DUMMY_EXTERNAL)
          .contains(HealthcheckApplication.DUMMY_INTERNAL);
    }
  }
  ...
  void shouldBeUnhealthyInternal(StdOut stdOut) {
    ...
    try (Response response = healthCheckGet("/healthcheck/internal")) {
      ...
      assertThat(logs)
          .contains("Internal healthcheck failed: " + HealthcheckApplication.DUMMY_INTERNAL);
    }
  }
  ...
  private void setApplicationHealth(boolean isHealthy) {
    HealthcheckApplication app = EXT.getApplication();
