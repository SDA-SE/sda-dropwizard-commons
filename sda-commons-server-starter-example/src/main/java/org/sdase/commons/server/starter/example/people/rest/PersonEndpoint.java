 ...
 */
@Api // identify this class for Swagger documentation, details about Swagger documentation follow in another example
...
public class PersonEndpoint {
   ...
   /**
    * Information about the requested URI. Jersey will inject a Proxy of {@code UriInfo} that is aware of the request
    * scope.
    */
   @Context
   ...
   /**
    * Creates a {@link PersonResource} representing the given {@link PersonEntity}. We do not want to expose the
    * underlying entities to our consumers. This way our API is independent from the internal model which may be changed
    * with an appropriate migration at any time. Especially when using HAL resources, we have the {@code _links} as
    * dynamic content that is not persisted with our entity.
    *
    * @param personEntity the entity that should be exposed as {@link PersonResource}
    * @return the {@link PersonResource} containing links to the {@linkplain PersonEntity#getChildren() children} and
    *         {@linkplain PersonEntity#getParents() parents} of the {@code personEntity} instead of embedding all the
    *         data.
    */
   private PersonResource toResource(PersonEntity personEntity) {
      ...
      if (personEntity.getChildren() != null && !personEntity.getChildren().isEmpty()) {
         personResource.setChildrenLinks(personEntity.getChildren().stream()
      ...
      if (personEntity.getParents() != null && !personEntity.getParents().isEmpty()) {
         personResource.setParentsLinks(personEntity.getParents().stream()
               .map(PersonEntity::getId)
               .map(this::personLocation)
               .map(HALLink.Builder::new)
               .map(HALLink.Builder::build)
   ...
   /**
    * Creates the location to the {@link PersonEntity} with the given {@code id}. The location is based on the base URI
    * of the request context and therefore will contain the same protocol, host and port the client used in the current
    * request.
    *
    * @param id the {@linkplain PersonEntity#getId() id} of the {@link PersonEntity} the link should be created for.
