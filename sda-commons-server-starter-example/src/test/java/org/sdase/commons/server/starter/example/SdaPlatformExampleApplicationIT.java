...
public class SdaPlatformExampleApplicationIT {
   ...
   // create a dummy authentication provider that works as a local OpenId Connect provider for the tests
   ...
   public static final DropwizardAppRule<SdaPlatformConfiguration> DW =
         ...
         DropwizardRuleHelper.dropwizardTestAppFrom(SdaPlatformExampleApplication.class)
               ...
               .withRandomPorts()
               // apply the auth config to the test instance of the application to verify incoming tokens correctly
   ...
   public void rejectApiRequestWithoutAuthentication() {
      Response response = baseUrlWebTarget().path("people").request(APPLICATION_JSON) // NOSONAR
            .header(ConsumerTracing.TOKEN_HEADER, TEST_CONSUMER_TOKEN).get();
   ...
   public void rejectApiRequestWithoutConsumerToken() {
      Response response = baseUrlWebTarget().path("people").request(APPLICATION_JSON)
            .headers(AUTH.auth().buildAuthHeader()).get();
   ...
   public void respond404ForUnknownPerson() {
      Response response = baseUrlWebTarget().path("people").path("jamie-doe").request(APPLICATION_JSON)
            .header(HttpHeaders.AUTHORIZATION, AUTH.auth().buildHeaderValue())
            .header(ConsumerTracing.TOKEN_HEADER, TEST_CONSUMER_TOKEN)
            .get();
   ...
   public void provideSelfLinkInPersonResource() {
      PersonResource personResource = baseUrlWebTarget().path("people").path("john-doe").request(APPLICATION_JSON)
            .header(HttpHeaders.AUTHORIZATION, AUTH.auth().buildHeaderValue())
            .header(ConsumerTracing.TOKEN_HEADER, TEST_CONSUMER_TOKEN)
   ...
   public void provideRelationLinksInPersonResource() {
      PersonResource personResource = baseUrlWebTarget().path("people").path("jasmine-doe").request(APPLICATION_JSON)
            .header(HttpHeaders.AUTHORIZATION, AUTH.auth().buildHeaderValue())
            .header(ConsumerTracing.TOKEN_HEADER, TEST_CONSUMER_TOKEN)
            .get(PersonResource.class);
      ...
      assertThat(actualParentsLinks).extracting(HALLink::getHref)
