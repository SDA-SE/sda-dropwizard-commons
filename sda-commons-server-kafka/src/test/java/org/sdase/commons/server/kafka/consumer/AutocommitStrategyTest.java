...
public class AutocommitStrategyTest {
  ...
  public void itShouldReenterPollingQueue() {
    ...
    Mockito.verify(consumer, timeout(WAIT_TIME_MS).atLeast(2)).poll(argThat(d -> d.toMillis() > 0));
  }
  ...
  public void errorHandlerShouldBeInvokedWhenExceptionButNotStop() {
    ...
    when(consumer.poll(argThat(d -> d.toMillis() > 0))).thenReturn(records);
  ...
  public void itShouldHandAllRecordsToMessageHandler() {
    ...
    when(consumer.poll(argThat(d -> d.toMillis() > 0)))
  ...
  public void itShouldCorrectlyStop() {
    ...
    Mockito.verify(consumer, timeout(WAIT_TIME_MS).times(1)).poll(argThat(d -> d.toMillis() > 0));
