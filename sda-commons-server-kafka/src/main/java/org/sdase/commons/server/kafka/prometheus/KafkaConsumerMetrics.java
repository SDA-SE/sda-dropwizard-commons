...
/**
 * Custom Prometheus Collector scraping Kafka Metrics for all registered message
 * listeners.<br>
...
public class KafkaConsumerMetrics extends Collector {
   ...
   /**
    * <a href=
    * "https://kafka.apache.org/documentation/#consumer_monitoring">Kafka
    * Metrics</a> scraped by this class.
    */
   private static final String[] KAFKA_METRICS = { "records-lag" };

   /**
    * The labels added by {@link KafkaConsumerMetrics}. The labels and their
    * order have to be aligned with the values created in
    * {@link #createLabelValuesForCurrentMessage(String, String, String)}
    */
   ...
   /**
    * Creates and registers a custom Prometheus Collector {@link Collector}.
    * <strong>Note that there should be only one registered instance of this
    * type in the application.</strong>
    * 
   ...
   public List<MetricFamilySamples> collect() {
      ...
      messageListeners.forEach(listener -> mfs.addAll(collectGaugesPerListener(listener.getConsumer().metrics())));
      return mfs;
   }
   ...
   private List<GaugeMetricFamily> collectGaugesPerListener(Map<MetricName, ? extends Metric> metrics) {
      ...
      Arrays.stream(KAFKA_METRICS).forEach(
   ...
   private GaugeMetricFamily collectKafkaMetric(Entry<MetricName, ? extends Metric> kafkaMetric) {
      ...
      GaugeMetricFamily labeledGauge = new GaugeMetricFamily(normalizedMetricName, kafkaMetric.getKey().description(),
            Arrays.asList(LABELS));

      String[] labelValues = createLabelValuesForCurrentMessage(
            kafkaMetric.getKey().tags().get(KAFKA_METRIC_TAG_CLIENT_ID));

      labeledGauge.addMetric(Arrays.asList(labelValues), (double) kafkaMetric.getValue().metricValue());
      return labeledGauge;
   }

   /**
    * Creates all values for the labels required by the gauges in appropriate
    * order.
    * 
    * @param consumerName
    * @return all values for the labels in the order they are registered in the
    *         respective gauge
    */
    ...
    */
   @SuppressWarnings("static-method")
   private Entry<MetricName, ? extends Metric> metricByNameOrNull(Map<MetricName, ? extends Metric> metrics,
         String name) {
      return metrics.entrySet().stream().filter(mx -> mx.getKey().name().equals(name)).findFirst().orElse(null);
   }
}
