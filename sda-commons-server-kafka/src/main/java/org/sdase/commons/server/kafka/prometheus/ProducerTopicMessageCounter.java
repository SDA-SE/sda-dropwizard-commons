...
public class ProducerTopicMessageCounter {
   ...
   * aligned with the values created in {@link #createLabelValuesForCurrentRequest(String, String)}
   */
  ...
  /**
   * Creates and registers a new {@link Counter} matching the specification of this {@code
   * TopicCounterSpecification} instance. <strong>Note that there should be only one registered
  ...
  public void unregister() {
    try {
      ...
      LOGGER.error("Tried to remove collector that is not registered: " + topicMessagesCounter.toString());
    }
  }
  ...
  /**
   * Creates all values for the labels required by the counter in appropriate order.
   *
  ...
  private Counter createAndRegister() {
    Counter.Builder topicMessageCounterBuilder =
        Counter.build().name(METRIC_NAME).labelNames(LABELS).help(HELP);
