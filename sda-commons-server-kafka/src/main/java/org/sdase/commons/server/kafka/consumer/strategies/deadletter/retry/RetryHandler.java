...
/**
 * This class is used for sending messages to a kafka topic with a specified delay. The message
 * handler thread will sleep for a given time if message timestamp is not smaller than current time
 * - delay
 */
public class RetryHandler implements ErrorHandler<byte[], byte[]>, MessageHandler<byte[], byte[]> {
  ...
  /**
   * Stops the consumer for the retry interval and then inserts the record to the main topic. By
   * doing so, the retry is triggered not before retry interval expires.
   *
  ...
  /**
   * Processes a record read from retry topic. If the timestamp of the message is not smaller than
   * current time minus retry interval, an {@link DelayRetryException} is thrown to force the
   * invocation of the error handler that again invokes the 'handleError' method.
   *
