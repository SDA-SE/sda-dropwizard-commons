...
public class HibernateExampleApplication extends Application<HibernateExampleConfiguration> {
   ...
   /**
    * initialization of the Hibernate bundle. Be sure to import
    * {@link HibernateBundle} (from sda-commons) and not
    * {@link io.dropwizard.hibernate.HibernateBundle} (default dropwizard)
    */
   private final HibernateBundle<HibernateExampleConfiguration> hibernateBundle = HibernateBundle
         .builder()
   ...
   public void run(HibernateExampleConfiguration configuration, Environment environment) {
      ...
      environment.jersey().register(new PersonsEndPoint(new PersonManager(hibernateBundle.sessionFactory())));
   }
