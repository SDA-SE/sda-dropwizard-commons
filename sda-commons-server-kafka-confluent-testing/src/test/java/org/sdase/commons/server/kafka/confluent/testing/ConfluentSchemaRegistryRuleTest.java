...
public class ConfluentSchemaRegistryRuleTest {
  ...
  @ClassRule
  public static final ConfluentSchemaRegistryRule SCHEMA_REGISTRY =
  ...
  /**
   * Publishes a message to the Kafka topic with an @{@link
   * io.confluent.kafka.serializers.KafkaAvroSerializer}. During this, the schema is published to
   * the registry internally. Checks if the schema is stored successfully within the registry. Also
   * verifies that the message is within the topic
   */
  @Test
  public void schemaShouldBeRegistered() throws SchemaRegistryException {
    ...
    props.put(
        CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, kafkaTestResource.getKafkaConnectString());
    ...
    Iterator<io.confluent.kafka.schemaregistry.client.rest.entities.Schema> allSchemaVersions =
        SCHEMA_REGISTRY.getAllSchemaVersions("topic1-value");
    Assert.assertTrue(allSchemaVersions.hasNext());

    List<ConsumerRecord<byte[], byte[]>> topic1 =
        kafkaTestResource.getKafkaTestUtils().consumeAllRecordsFromTopic("topic1");
