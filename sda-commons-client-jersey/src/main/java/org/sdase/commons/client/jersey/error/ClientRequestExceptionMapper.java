...
/**
 * Exception mapper that converts {@link ClientRequestException} thrown from an outgoing request to
 * another service into a {@code 500 Internal Server Error} response for our client.
 */
public class ClientRequestExceptionMapper implements ExceptionMapper<ClientRequestException> {
  ...
  private static final String CONNECT_TIMEOUT_SUBTITLE =
      "Connect timeout when invoking another service.";
  private static final String PROCESSING_SUBTITLE =
      "Processing failed when invoking another service.";
