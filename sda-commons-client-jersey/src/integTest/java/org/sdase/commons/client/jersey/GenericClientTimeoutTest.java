...
public class GenericClientTimeoutTest {
   ...
   public void runIntoDefaultConnectionTimeoutOf500Millis() {
      ...
      await()
            ...
            .untilAsserted(() -> assertThatExceptionOfType(ClientRequestException.class).isThrownBy(() ->
            // try to connect to an ip address that is not routable
            convertExceptions(() -> client.target("http://192.168.123.123").path("timeout").request().get() // NOSONAR
   ...
   public void runIntoConfiguredConnectionTimeout() {
      ...
      await()
            ...
            .untilAsserted(() -> assertThatExceptionOfType(ClientRequestException.class).isThrownBy(() ->
            // try to connect to an ip address that is not routable
            convertExceptions(() -> client.target("http://192.168.123.123").path("timeout").request().get()))
                  .is(timeoutError())
   ...
   public void runIntoDefaultReadTimeoutOf2Seconds() {
      ...
      await()
            ...
            .untilAsserted(() -> assertThatExceptionOfType(ClientRequestException.class).isThrownBy(() ->
            // try to connect to an ip address that is not routable
            convertExceptions(() -> client.target(WIRE.baseUrl()).path("timeout").request().get()))
                  .is(timeoutError())
                  .is(readTimeoutError()));
   }
