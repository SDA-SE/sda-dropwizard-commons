...
public class RequestDurationHistogramSpecification {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(RequestDurationHistogramSpecification.class);
   ...
   * aligned with the values created in {@link #createLabelValuesForCurrentRequest(ResourceInfo,
   ...
   * RequestDurationHistogramSpecification} instance. <strong>Note that there should be only one
   * registered instance of this type in the application.</strong>
   */
  ...
  public void unregister() {
    try {
      ...
      LOGGER.error("Tried to remove collector that is not registered: " + requestDurationHistogram.toString());
    }
  }

  /**
   * Observes the given request duration and adds the defined labels according to the request and
   * response context information.
   *
   * @param requestDurationSeconds the duration of the current request in seconds
   * @param resourceInfo the {@link ResourceInfo} of the current request to determine values for the
   *     labels
  ...
  void observe(
    ...
    String[] labelValues =
        createLabelValuesForCurrentRequest(resourceInfo, requestContext, responseContext);
  ...
  private Histogram createAndRegister() {
    Histogram.Builder requestDurationHistogramBuilder =
  ...
  private String getConsumerName(ContainerRequestContext requestContext) {
    ...
    return consumerName != null ? consumerName.toString() : "";
  }
  ...
  private static String restorePathParamPlaceholders(
      String requestPath, Map<String, List<String>> pathParameters) {
    if (requestPath == null) {
      return null;
    }
