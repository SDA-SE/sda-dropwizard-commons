...
/**
 * Checks that custom error mappers are registered by the JacksonConfigurationBundle. The check is
 * indirectly performed by checking that the bundle itself is registered. This class checks for the
 * risks identified in the security guide as:
 *
 * <ul>
 *   <li>"Risiko: Erkennung von vertraulichen Komponenten ... Entfernen von applikations-bezogenen
 *       Fehlermeldungen"
 *   <li>"Risiko: Erkennung von vertraulichen Komponenten ... Zentrales Abfangen aller Exceptions"
 * </ul>
 */
public class CustomErrorHandlerSecurityAdvice {
  ...
  private void forceDefaultExceptionMappersAreNotRegistered() {
    if (Boolean.TRUE.equals(abstractServerFactory.getRegisterDefaultExceptionMappers())) {
      LOG.info(
          "Disabling registerDefaultExceptionHeaders to avoid giving information to possible attackers.");
  ...
  private List<Class<?>> extractRegisteredBundleTypes(Class<?> classOrSuperClass) {
    ...
    try {
      ...
      for (Field bundleField : bundleFields) {
        ...
        registeredBundles.addAll(
            bundles.stream().map(Object::getClass).collect(Collectors.toList()));
      }
